...
async function generateResponseSchemasDefinitions(tsDocSettings) {
  const files = await getFilesFromPattern(tsDocSettings.filesPattern)
  const settings = {
    required: true,
    include: files,
    excludePrivate: true
  ...
  const program = TJS.getProgramFromFiles(resolved, tsDocSettings.tsConfig.compilerOptions);
  ...
  const symbols = generator.getSymbols()
    ...
    .filter((s => s.name.includes(tsDocSettings.symbolPattern)))
...
exports.plugin = {
  ...
  register: async (server, options) => {
    ...
    if (options.tsDocSettings) {
      settings.definitionsToMerge = await generateResponseSchemasDefinitions(options.tsDocSettings)
    }
