...
function generateResponseSchemas(tsConfig, files) {
  const settings = {
    required: true,
    include: files,
    excludePrivate: true
  ...
  const program = TJS.getProgramFromFiles(resolved, tsConfig.compilerOptions);
  ...
  return generator.getSchemaForSymbols(symbols.map(s => s.name))
}
...
exports.plugin = {
  ...
  register: (server, options) => {
    ...
    const schemas = generateResponseSchemas(options.tsConfig, options.includeFiles)
    ...
    settings.controllersSchemas = schemas;
